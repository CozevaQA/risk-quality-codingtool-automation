...
def measure_search_registry(driver, measure, abb):
...
def measure_patient_finder(driver, customer, lob, measure_name, measure_abb,domain_name, ws, wb):
    ...
    try:
        ...
        try:
            ...
            rows = driver.find_element(By.XPATH, "//*[@id='quality_registry_list']").find_element(By.TAG_NAME,
                                                                                                  'tbody').find_elements(
                By.TAG_NAME, 'tr')
            try:
                patient_link = rows[0].find_element(By.XPATH, "//*[contains(@class,'pat_name')]").get_attribute('href')
...
#                 break
#     except Exception as e:
#         print("Error Occured at metric match!")
...
def code_search_coding_tool(driver, customer, lob, measure_name, ws):
        ...
        for metric in metric_rows:
            try:
                metric_name = metric.find_element(
                    By.XPATH,
                    ".//*[contains(@class,'met-name')]").text.strip()

                if measure_name == metric_name:
                    target_metric = metric
                    print("Matched metric: " + metric_name)
                    driver.execute_script("arguments[0].scrollIntoView({block:'center'});",target_metric)
                    driver.find_element(By.XPATH,"//*[@id='right_panel_nav_tabs']/li[1]/div").click()
                    time.sleep(1)
                    add_more_btn = target_metric.find_element(By.XPATH,".//*[contains(@class,'add_more')]")
                    add_more_btn.click()
                    time.sleep(2)
                    break
            except Exception as inner_e:
                print("Skipping metric due to error:", inner_e)
    except Exception as e:
        print("Error Occured at metric match!")
        return
    ...
    choices = driver.find_elements(By.XPATH,"//*[contains(@class,'choice_tr')]//*[@class='choice_option']//ul/li")
    print("Executing code search...")
    for i in range(0, len(choices), 1):
        driver.find_element(By.XPATH,"//*[contains(@class,'choice_tr')]//*[@class='choice_option']//*[@class='select-wrapper']").click()
        time.sleep(2)
        ...
        result_list = []
        try:
            # FIND ALL ADVANCED SEARCH BUTTONS INSIDE TARGET METRIC
            adv_buttons = driver.find_elements(By.XPATH, "//*[@class='advanced_search_btn']")
            if len(adv_buttons) == 0:
            ...
            for btn in adv_buttons:
                btn.click()
                populated_list = driver.find_elements(By.XPATH, "//*[@id='code_ul']/li")
                code_id_value_list = []
                result_list = []
                try:
                    for element in populated_list:
...
ws.append([
    ...
    "MSPL Pencil Icon Present (Y/N)",
    "MSPL Pencil Icon Options",
    "Dashboard Pencil Icon Present (Y/N)",
    "MAP Checkbox Present in Dashboard (Y/N)",
    "MSPL vs Dashboard MAP Alignment (Pass/Fail)",
    "MSPL vs Dashboard Pencil Alignment (Pass/Fail)",
    "MSPL Link"
])
format_excel_sheet(ws)
...
for record in lob_cust_mapping:
    ...
    try:
        ...
        print(e)
        print("Error occured at main block!")
